...
**How it works**
- Reads `extra_usage.is_enabled`, `utilization`, `used_credits`, and `currency` from `~/.claude/.statusline-usage-cache.json` — the same background-polled cache used by `/statusline-model-usage`. No extra API calls.
